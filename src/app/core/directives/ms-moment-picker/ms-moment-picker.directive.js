    ...
    var MomentPickerDirective = (function () {
        ...
        MomentPickerDirective.prototype.link = function ($scope, $element, $attrs) {
            $scope.template = (
                ...
                '<thead>' +
                '<tr>' +
                '<th ng-class="{disabled: !view.previous.selectable}" ng-bind-html="view.previous.label" ng-click="view.previous.set()" md-prevent-menu-close></th>' +
                '<th ng-bind="view.title" ng-click="view.setParentView()" md-prevent-menu-close></th>' +
                '<th ng-class="{disabled: !view.next.selectable}" ng-bind-html="view.next.label" ng-click="view.next.set()" md-prevent-menu-close></th>' +
                '</tr>' +
                '</thead>' +
                '</table>' +
                ...
                '<td ng-repeat="day in days track by day.date" ng-class="day.class" ng-bind="day.label" ng-click="monthView.setDate(day)" md-prevent-menu-close></td>' +
                '</tr>' +
                '</tbody>' +
                '</table>' +
                ...
                '</table>' +
                '</div>' +
                '<div class="moment-picker-timepicker" layout="row" layout-align="space-between center">'+
                '<div>'+
                '<input type="number" ng-model="timepicker.hours" placeholder="00" min="01" max="12" ng-change="timepicker.setTime(timepicker.hours, timepicker.minutes, timepicker.pm)" />'+
                '</div>'+
                '<div>'+
                '<input type="number" ng-model="timepicker.minutes" placeholder="00" min="00" max="59" ng-change="timepicker.setTime(timepicker.hours, timepicker.minutes, timepicker.pm)" />'+
                '</div>'+
                '<div class="marker" ng-class="{pm:timepicker.pm, am:!timepicker.pm}" ng-click="timepicker.pm=!timepicker.pm;timepicker.setTime(timepicker.hours, timepicker.minutes, timepicker.pm)" md-prevent-menu-close>'+
                '<span class="marker-am">AM</span>'+
                '<span class="marker-pm">PM</span>'+
                '</div>'+
                '</div>'+
                '</div>'
            );
            ...
            // timepicker
            $scope.timepicker = {
                setTime: function (hours, minutes, pm) {
                    $scope.view.moment.set({
                        hours: hours?parseInt(hours)+(pm?12:0):0,
                        minutes: parseInt(minutes) || 0
                    });
                    $scope.view.update($scope.view.moment);
                    $scope.valueUpdate($scope.valueMoment = $scope.view.moment.clone());
                }
            };
